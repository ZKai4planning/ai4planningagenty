  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (step === "SUBMIT") {
      ...
      try {
        ...
        if (res.status === 423) {
          ...
          setLockedHours(data.retryAfterHours);
          setIsLocked(true);
          return;
        }
    ...
    if (step === "VERIFY_OTP") {
      ...
      try {
        ...
        setErrorMessage(errRes?.data?.message || "OTP verification failed");
        return;
      }
      ...
      if (res.status === 200) {
        ...
        if (!rawUserId) {
          try {
            ...
            rawUserId = payload?.userId ?? null;
          } catch {
            // Ignore token parse errors and fall back to unknown user id.
          }
        }
